...
class Reputation(object):
    def __init__(self, product_id, path) -> None:
        self.session = None
        self.header = header = {            
    ...
    def main(self):        
        self.header['referer'] = self.referer.format(product_id=self.product_id)
        collect_parser = {}
        for page in range(1, 35):            
            url = self._url.format(product_id=self.product_id, page=page)
...
if __name__ == '__main__':
    ...
    if not x:
        ...
        x = requests.post(url='http://localhost:8000/review/post/', data=json.dumps({'url': url, 'path': path}))
    else:
        
        res = requests.get(url='http://localhost:8000/review/get')
        data = json.loads(res.content.decode('utf-8'))['data']
        for d in data:                
            r = Reputation(d['product_id'], d['path'])
            result = r.main()
            print(result)
            if result:
                now_at = datetime.now().strftime('%Y%m%d %H:%M')
                consquence = requests.put(url='http://localhost:8000/review/put/', data=json.dumps({'try_at': now_at, 'product_id': d['product_id'], 'is_success': True}))
                print(consquence)
                # if consquence:
                #     continue
                # else:
